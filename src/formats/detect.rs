...
fn looks_like_utf16_html(bytes: &[u8], little_endian: bool) -> bool {
    ...
    while pairs
        .get(index)
        .is_some_and(|pair| utf16_ascii_unit(*pair, little_endian).is_some_and(|b| b.is_ascii_whitespace()))
    {
        index += 1;
    }
...
fn utf16_prefix_eq_ignore_ascii_case(
    ...
    slice.iter().zip(prefix).all(|(pair, expected)| {
        utf16_ascii_unit(*pair, little_endian).is_some_and(|byte| byte.eq_ignore_ascii_case(expected))
    })
}

fn utf16_html_prefix(
    pairs: &[[u8; 2]],
    start: usize,
    little_endian: bool,
    prefix: &[u8],
